...
int main() {
    ...
    for (int i = 0; i < kIters; ++i) {
        ...
        const MlKem768Status st_enc = internal::mlkem768_provider_encapsulate_by_id(
            MlKem768ProviderId::dna, kp.public_key, &enc);
        ...
        if (!expect_status("native decapsulate from dna ct", st_dec, MlKem768Status::ok)) {
            return 1;
        }
    ...
    std::cout << "[dna-pqcore] dna encaps interop ok"
              << " iters=" << kIters
